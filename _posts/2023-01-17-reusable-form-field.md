...
리액트는 이러한 부모가 자식을 제어하는 구조를 권장하지않고, 예상 가능한 데이터의 흐름과 구조의 단순화를 구해 단방향 데이터 흐름을 통해 컴포넌트를 설계하기를 권장한다.
...
예전에 경험했던 프로젝트에서 입력 양식을 이러한 고려 없이 관리하여, 수십개의 동일한 브랜드 검색 기능을 지니는 폼을 일괄 수정해야할 때 팀의 사기가 저하되는 경험을 한적이 있다.
